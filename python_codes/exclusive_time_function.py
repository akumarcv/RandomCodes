...
def exclusive_time(n, logs):  # Function to calculate exclusive time for each function
    ...
    for i in range(len(logs)):  # Iterate through each log entry
        if "start" in logs[i]:  # If the log entry is a start log
            ...
            duration = int(logs[i].split(":")[-1])-int(splits[-1])+1  # Calculate execution time
            result[int(splits[0])] +=duration  # Add duration to the function's total time
            if stack:  # If there are other functions on the stack
                result[int(stack[-1].split(":")[0])] -= (int(logs[i].split(":")[-1]) - int(splits[-1]) + 1)  # Subtract time from parent function
...
def main():  # Main function to test the exclusive_time function
    ...
    logs = [["0:start:0", "1:start:2", "1:end:3", "2:start:4", "2:end:7", "0:end:8"],  # Test case 1
        ["0:start:0", "0:start:2", "0:end:5", "1:start:6", "1:end:6", "0:end:7"],  # Test case 2
        ["0:start:0", "1:start:5", "1:end:6", "0:end:7"],  # Test case 3
        ["0:start:0", "1:start:5", "2:start:8", "3:start:12", "4:start:15", "5:start:19", "5:end:22", "4:end:24", "3:end:27", "2:end:32", "1:end:35", "0:end:36"],  # Test case 4
    ...
    for i in range(len(n)):  # Iterate through each test case
        ...
        print("\tOutput: ", exclusive_time(n[i], logs[i]), sep = "")  # Print the output of exclusive_time function
