...
class SelfAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        q_proj = self.q_linear(q).view(B, T, self.n_heads, self.proj_dim//self.n_heads).permute(0, 2, 1, 3) # proj_dim is divisible by n_heads
        k_proj = self.k_linear(k).view(B, T, self.n_heads, self.proj_dim//self.n_heads).permute(0, 2, 1, 3)
        v_proj = self.v_linear(v).view(B, T, self.n_heads, self.proj_dim//self.n_heads).permute(0, 2, 1, 3)
