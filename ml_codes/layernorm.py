...
class UnitTests:
    ...
    def test_layernorm_normalization():
        ...
        for i in range(batch_size):
            ...
            assert abs(sample_mean) < 1e-5, f"Mean of sample {i} is {sample_mean}, expected close to 0"
            assert abs(sample_std - 1.0) < 1e-4, f"Std of sample {i} is {sample_std}, expected close to 1"
    ...
    def test_against_pytorch_implementation():
        ...
        assert diff < 1e-5, f"Max difference between implementations is {diff}, expected less than 1e-5"
...
def compare_models():
    ...
    model_with_torch_ln = SimpleConvNet(use_layer_norm=True, use_custom_layer_norm=False).to(device)
    model_with_custom_ln = SimpleConvNet(use_layer_norm=True, use_custom_layer_norm=True).to(device)
    ...
    # Track metrics
    models = [
        {"name": "Without LayerNorm", "model": model_without_ln, "optimizer": optimizer_without_ln},
        {"name": "With PyTorch LayerNorm", "model": model_with_torch_ln, "optimizer": optimizer_with_torch_ln},
        {"name": "With Custom LayerNorm", "model": model_with_custom_ln, "optimizer": optimizer_with_custom_ln}
    ]
