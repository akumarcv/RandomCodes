...
def visualize_results(
    X_train, y_train, X_test, test_labels, dataset_name, output_file=None
):
...
def compare_weighting_schemes(X_train, y_train, X_test, y_test, dataset_name, k=5):
    ...
    fig.suptitle(
        f"Comparison of Weighting Schemes for {dataset_name} Dataset (k={k})",
        fontsize=16,
    )
    ...
    for i, weight_func in enumerate(weight_functions):
        ...
        ax.set_title(
            f"{weight_func.capitalize()} Weighting (Accuracy: {accuracy:.2f}%)"
        )
    ...
    # Save the figure
    output_file = (
        "/Users/amitkumar/Desktop/Preparation/ml_codes/knn_weighting_comparison.png"
    )
    plt.savefig(output_file)
    plt.show()
...
def parse_arguments():
    ...
    # Model arguments
    parser.add_argument("--k", type=int, default=5, help="Number of neighbors to use")
    parser.add_argument(
        "--weight-function",
        type=str,
        default="inverse",
...
def main():
    ...
    if args.compare_weights:
        ...
        print(
            f"Visualization saved to: /Users/amitkumar/Desktop/Preparation/ml_codes/knn_weighting_comparison.png"
        )
